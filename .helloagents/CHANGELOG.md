...
### 新增
- 测试：新增 Android `LiaoWebSocketClientTest`，覆盖连接建立、重复连接守卫、旧连接替换、sign/私聊/提示/改名/改资料指令发送、入站消息/bytes 事件分发、forceout 与关闭失败 fallback / 重连调度分支；2026-03-17 复跑 `cd android-app && ./gradlew testDebugUnitTest jacocoDebugUnitTestReport --no-daemon` 后，Android Debug Unit Test 覆盖率提升至 `line 75.19% / branch 44.16% / method 76.59% / class 82.11%`，当前累计 47 个测试源文件 / 292 条用例，`LiaoWebSocketClient` branch 提升至 `61.99%`。
- 测试：新增 Android `VideoExtractCreateRepositoryTest` 与 `VideoExtractCreateViewModelTest`，覆盖上传元数据解析、multipart 构造、probe/createTask payload 组装与错误兜底，以及创建页上传/探测/创建守卫分支；2026-03-17 复跑 `cd android-app && ./gradlew testDebugUnitTest jacocoDebugUnitTestReport --no-daemon` 后，Android Debug Unit Test 覆盖率提升至 `line 71.51% / branch 42.28% / method 74.21% / class 81.05%`，当前累计 46 个测试源文件 / 286 条用例，`VideoExtractCreateRepository` branch 提升至 `70.24%`，`VideoExtractCreateViewModel` 提升至 `100%`。
- 测试：继续补充 Android `ChatRoomViewModelTest`，覆盖历史分页并发守卫、媒体面板缓存守卫、通知/强制下线、非当前会话消息忽略、空 peer 兜底与失败重试 fallback clientId 分支；2026-03-17 复跑 `cd android-app && ./gradlew testDebugUnitTest jacocoDebugUnitTestReport --no-daemon` 后，Android Debug Unit Test 覆盖率提升至 `line 68.63% / branch 40.81% / method 71.24% / class 78.25%`，当前累计 44 个测试源文件 / 273 条用例，`ChatRoomViewModel` branch 提升至 `76.39%`。
