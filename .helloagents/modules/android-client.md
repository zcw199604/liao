...
## 测试覆盖现状
- 当前 Android 侧已有 28 个本地单元测试文件（共 184 条用例）：在原有协议/基础映射测试之外，新增 `ChatRoomFeatureHelpersTest.kt`、`ChatRoomRepositoryTest.kt`、`DouyinRepositoryMappingsTest.kt`、`DouyinRepositoryBranchTest.kt`、`MtPhotoRepositoryMappingsTest.kt`、`MtPhotoRepositoryBranchTest.kt`，并延续 `VideoExtract*`、`Douyin*`、`MtPhoto*` 等既有回归。
- 已覆盖：协议辅助函数（MD5 自发消息判定、`forceout` 5 分钟禁连）、WS `code/act` 目录解析、`privateMessage` act 组装、协议用户显示名兜底、私聊 envelope → 时间线转换、消息类型推断、文件名提取、消息预览文案、`IdentityDto/ChatUserDto/ChatMessageDto/HistoryMessageDto` 映射、历史消息 / 全局收藏 JSON 解析、匹配候选与重连退避上限、`generateCookie` / `generateRandomIp` / `normalizeTextForMatch` 与缓存快照序列化稳定性、API Base URL / WebSocket URL / 动态 Base URL 重写规则、媒体 MIME/标签与上传路径归一化，以及 Auth / Identity / ChatList / ChatRoom / GlobalFavorites / MediaLibrary / Settings / Douyin / mtPhoto / VideoExtract Repository 与 helper 的成功 / 失败 / fallback / 缓存更新 / 文案映射 / 操作分支。
- 未覆盖：Compose UI / 导航交互、`src/androidTest/` 仪器测试、Room/DataStore 持久化集成、Hilt 注入、真实网络与 WebSocket 生命周期、多设备并发 / 弱网场景。
...
- 当前判断：Android 已建立“协议对齐 + Repository / helper JVM 回归”的第三轮基线，并已完成 branch ≥ 30% 目标，但 UI 与集成测试矩阵仍需后续持续补齐。
