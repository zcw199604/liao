# 数据模型（MySQL / PostgreSQL / Redis / 内存缓存）

> 本文档整理当前 Go 后端使用到的主要数据表与缓存结构；历史 Java(Spring Boot) 源码已从仓库移除，文中提到“原 Java 行为”仅用于兼容性说明。表结构迁移见 `internal/database/migrator.go` + `sql/{dialect}/*.sql`（入口：`internal/app/schema.go`）；缓存实现见 `internal/app/user_info_cache*.go`、`internal/app/image_cache.go`、`internal/app/forceout.go`。
...
## 1. 数据库表（MySQL / PostgreSQL）

说明：
- 服务启动时会根据 `DB_URL` 的 scheme 选择数据库类型（MySQL / PostgreSQL），并通过 `internal/database/migrator.go` 执行 `sql/{dialect}/*.sql` 完成建表/迁移（入口：`internal/app/schema.go`）。
- MySQL 的 `AUTO_INCREMENT` 在 PostgreSQL 中对应 `GENERATED BY DEFAULT AS IDENTITY`（语义相同：自增主键）。
...
**创建位置**：`sql/{dialect}/*.sql`（由 `internal/database/migrator.go` 执行；入口：`internal/app/schema.go`）  
**历史参考**：`sql/init.sql`（旧 MySQL 初始化脚本，不作为运行时迁移的 SSOT）  
...
**创建位置**：`sql/{dialect}/*.sql`（由 `internal/database/migrator.go` 执行；入口：`internal/app/schema.go`；实际数据通常由外部扫描/入库流程写入）。
...
**跨库一致性说明**
- MySQL 常见 `utf8mb4_*_ci` collation 下，`name` 的唯一性与比较通常是“大小写不敏感”的。
- 为确保切换 PostgreSQL 后行为一致，PG 迁移会额外创建 `uk_dfut_name_ci (LOWER(name))`（见 `sql/postgres/003_tag_name_ci_unique.sql`），从而拒绝仅大小写不同的重复标签名。
...
**跨库一致性说明**
- 同 `douyin_favorite_user_tag.name`，PostgreSQL 侧通过 `uk_dfat_name_ci (LOWER(name))` 确保“大小写不敏感”的全局唯一性（见 `sql/postgres/003_tag_name_ci_unique.sql`）。
