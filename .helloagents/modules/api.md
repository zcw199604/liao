...
- 作为后端 HTTP/WebSocket 接口与处理逻辑的统一入口文档（SSOT）。
- 记录认证鉴权、WebSocket 代理、HTTP 接口清单与关键返回语义。
- 为模块文档与实现代码提供可追溯的接口事实基线。

## 行为规范

- 接口行为以 `internal/app/` 运行时实现为准；文档需与代码保持一致。
- HTTP 路径与鉴权规则按本文定义执行（尤其 `/api/**` 拦截例外与 401 语义）。
- WebSocket `/ws` 的 sign、转发、forceout、连接池行为按本文约束执行。

## 依赖关系

- 路由与处理器：`internal/app/router.go`、`internal/app/*_handlers.go`
- 鉴权链路：`internal/app/middleware.go`、`internal/app/jwt.go`、`internal/app/auth_handlers.go`
- WS 代理链路：`internal/app/websocket_proxy.go`、`internal/app/websocket_manager.go`、`internal/app/forceout.go`
- 前端调用入口：`frontend/src/api/*`、`frontend/src/composables/useWebSocket.ts`
...
**描述**：导出抖音“实况照片”（静态图 + 动态短视频）。支持两种输出格式：iOS Live Photo（ZIP：`.jpg` + `.mov` 配对）与 Motion Photo（单文件 `.jpg`，尾部附加 `.mp4` 并写入 XMP `GCamera:MicroVideoOffset`）。

**请求（query）**
| 参数 | 必填 | 说明 |
|---|---|---|
| format | 否 | `zip/jpg`，默认 `zip`。`zip`：iOS Live Photo；`jpg`：Motion Photo 单文件 |
| key | 是 | `/api/douyin/detail` 或 `/api/douyin/account` 返回的缓存 key |
...
**描述**：调用上游单页分页接口 `/douyin/account/page` 拉取该用户最新作品并合并入库，返回本次新增数量（用于前端弹窗提示）。
