...
- 维护前端主题（light/dark/auto）与排版规范的统一基线。
- 约束组件视觉 token、移动端排版与弹层样式一致性。
- 为 UI 收敛与样式回归提供可复用规则。

## 行为规范

- 普通容器与文案优先使用语义化 token（`bg-surface*` / `text-fg*` / `border-line*`）。
- 固定深色卡片需提供不透明深色基底并搭配固定白字体系，避免对比度失衡。
- 关键标题/按钮在移动端需避免被挤压成逐字换行，必要时使用响应式分行策略。

## 依赖关系

- 全局样式入口：`frontend/src/index.css`
- 关键组件：`frontend/src/components/settings/SettingsDrawer.vue`、`frontend/src/components/media/MediaPreview.vue`、`frontend/src/components/chat/ChatSidebar.vue`
- 主题与测试约束：`frontend/src/components/media/*`、`frontend/src/components/chat/*`
