...
- 功能行为、输入输出与约束以本文“需求”与“API接口”章节为准。
- 文档与代码冲突时以代码为准并及时回写文档。
...
- 入口C：全站媒体库（AllUploadImageModal）→ 媒体详情面板（MediaDetailPanel）点击作者（当 `authorSecUserId` 存在时）→ 先关闭预览层，再打开抖音弹窗并优先进入“收藏作者（users）”定位该作者的本地收藏作品详情（未命中时停留在收藏列表）
- 交互：
  - 模式A：作品解析
...
> 前端应优先使用 `items[].downloadUrl`（`/api/douyin/download`）进行缩略图与预览加载。
  ...
  - 模式C：收藏
    0) 从聊天页“抖音收藏作者”入口进入时，默认落在“用户收藏”（作者列表）
...
- 后端请求头策略（用于降低 `403` 概率，best-effort）：
  ...
  - 当下载直链 host 为 `*.douyin.com` / `*.iesdouyin.com` / `*.douyinvod.com` / `*.douyinpic.com` 时：best-effort 携带抖音 cookie（来源优先级同“CookieCloud 自动抖音 Cookie”）；并在重定向到其他 host（CDN）时自动清除 `Cookie` 头，避免泄漏。
