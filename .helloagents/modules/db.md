...
- 维护 MySQL / PostgreSQL 双栈切换与迁移的统一运行手册。
- 约束数据库选择、迁移执行与切换回滚的标准流程。
- 为运维与开发提供可复用的验收与排障清单。

## 行为规范

- 数据库类型仅通过 `DB_URL` scheme 选择，不引入额外运行开关。
- 启动后必须按 `schema_migrations + sql/{dialect}/*.sql` 执行迁移。
- 切换与迁移操作前后必须执行文档中的校验清单与回归步骤。

## 依赖关系

- 配置与方言：`internal/config/config.go`、`internal/database/dialect.go`、`internal/app/app.go`
- 迁移入口与执行：`internal/app/schema.go`、`internal/database/migrator.go`
- 迁移脚本：`sql/mysql/`、`sql/postgres/`
