...
- 缓存：`CACHE_TYPE`（memory/redis）、`UPSTASH_REDIS_URL`/`REDIS_URL`（支持 `rediss://`）、`REDIS_HOST`、`REDIS_PORT`、`REDIS_PASSWORD`、`REDIS_DB`、`CACHE_REDIS_FLUSH_INTERVAL_SECONDS`（写入批量 flush 间隔）、`CACHE_USERLIST_TTL_SECONDS`（历史/收藏列表本地缓存 TTL）
