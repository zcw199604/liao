...
- 缓存：`CACHE_TYPE`（memory/redis）、`UPSTASH_REDIS_URL`/`REDIS_URL`（支持 `rediss://`）、`REDIS_HOST`、`REDIS_PORT`、`REDIS_PASSWORD`、`REDIS_DB`、`CACHE_REDIS_EXPIRE_DAYS`（用户信息/最后消息 TTL）、`CACHE_REDIS_CHAT_HISTORY_PREFIX`、`CACHE_REDIS_CHAT_HISTORY_EXPIRE_DAYS`（聊天记录 TTL）、`CACHE_REDIS_FLUSH_INTERVAL_SECONDS`（写入批量 flush 间隔）、`CACHE_REDIS_LOCAL_TTL_SECONDS`（Redis 本地缓存 TTL）
