...
### 需求: 媒体预览视频操作
**模块:** Media
`frontend/src/components/media/MediaPreview.vue` 是图片、视频和文件的统一预览层。视频预览由 Plyr 增强原生 `video`，并提供播放/暂停、前后 1 秒、横滑 seek、竖滑音量、倍速/慢放、长按临时 2x、当前帧抓取和抽帧任务入口。

#### 场景: 视频抓帧
- 抓帧前先暂停当前视频并使用 canvas 绘制当前帧。
- 抓帧开始会清理待处理单击/双击 tap，暂停后主动同步播放状态，避免抓帧过程中再次触发播放/暂停翻转。
- 全屏抓帧 loading 期间保持浮层工具栏可见，抓帧结束后恢复自动隐藏。
- 抓帧结果会下载为 PNG；存在当前身份时会继续上传到图片库。
- 跨域视频无法 canvas 抓帧时，应提示用户改用本地库或抽帧任务。

#### 场景: 播放器状态同步
- 倍速、当前时间和音量写入同时同步 Plyr 与原生 `video`，避免抓帧、横滑 seek、慢放和长按 2x 看到的状态不一致。
