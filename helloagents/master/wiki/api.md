...
| GET | `/api/getMtPhotoFolderAlbumLinks` | 查询文件夹直接关联的相册映射 `items:{folderId:[album,...]}`，排除规则不计入；上游查询失败时返回错误 |
| POST | `/api/moveMtPhotoFolderFiles` | 移动源目录全部文件到目标目录；JSON：`sourceId,targetId,includeSubfolders`，默认直属文件，重名自动重命名 |
