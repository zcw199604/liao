...
- 前端/后端：修复抖音作品在站内预览/缩略图加载失败（但新标签可打开）的问题：前端预览/缩略图改用 `items[].downloadUrl`（`/api/douyin/download`）代理地址；后端放行 `/api/douyin/download` 以支持 `<img>/<video>` 直连，并透传 `Range` 与相关响应头以改善视频播放/拖动。
