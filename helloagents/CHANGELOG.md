...
- 后端：支持将聊天记录缓存到 Redis（默认 30 天，可通过 `CACHE_REDIS_CHAT_HISTORY_EXPIRE_DAYS` 配置），每会话一个 ZSET key；`/api/getMessageHistory` 优先返回 Redis（命中足够则跳过上游），不足时再请求上游并合并去重以弥补上游过期缺口并减少请求次数。
