...
func TestLoad_DefaultCookieCloudCookieExpireHoursWhenNonPositive(t *testing.T) {
	...
	if cfg.CookieCloudCookieExpireHours != 1 {
		t.Fatalf("CookieCloudCookieExpireHours=%d, want 1", cfg.CookieCloudCookieExpireHours)
	}
}

func TestLoad_DefaultCookieCloudCookieExpireHoursWhenUnset(t *testing.T) {
	t.Setenv("COOKIECLOUD_COOKIE_EXPIRE_HOURS", "")
	t.Setenv("COOKIE_CLOUD_COOKIE_EXPIRE_HOURS", "")
